...
6. Never make a public API change (removed/renamed exported identifier,
   ...
   section 5.
7. `openapi.yml` is generated by an agent from the actual server
   implementation, not hand-authored — it can be wrong (a field typed
   `number` that's actually sent as a decimal string, a required request
